...
async function refresh() {
  chrome.runtime.sendMessage({ type: "GET_STATUS" }, (res) => {
    ...
    const open = res.openPorts && res.openPorts.length ? " — open: " + res.openPorts.join(", ") : "";
    $("status").textContent = (res.connected ? "Connected to bridge" : "Disconnected") + open;
...
$("reconnect").addEventListener("click", () => {
  ...
  chrome.storage.local.set({ ports: ports.length ? ports : [8765, 8766], autoFocus: $("autofocus").checked }, () => {
